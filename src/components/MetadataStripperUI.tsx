...
export function MetadataStripperUI() {
    ...
    const processFile = useCallback(async (entry: FileEntry) => {
        ...
        try {
            ...
            setResults((prev) => [...prev.filter((r) => r.id !== entry.id), { id: entry.id, blob, name, size: blob.size }]);
        } catch (err) {
                ...
                <div className="mt-6 space-y-4">
                    ...
                    <div className="flex items-center justify-between gap-4">
                        <div className="text-xs text-slate-500 dark:text-slate-400">
                            {allDone ? (
                                ...
                                <span>
                                    <span className="font-semibold text-violet-400">{results.length}</span> of <span className="font-semibold">{files.length}</span> cleaned
                    ...
                    <div className="grid gap-3">
                        {files.map((file) => {
                            ...
                            const saved = result ? Math.round(((file.file.size - result.size) / file.file.size) * 100) : null;

                            return (
                                <div key={file.id} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/8 rounded-2xl p-4 flex flex-col gap-3 shadow-sm">
                                    <div className="flex items-center gap-4">
                                        {/* Thumbnail */}
                                        <div className="w-14 h-14 rounded-xl overflow-hidden bg-violet-950/30 shrink-0 ring-1 ring-violet-100 dark:ring-violet-900">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs">
                                                ...
                                                {!isProcessing && fileError && <span className="text-red-500 dark:text-red-400 font-medium">Failed</span>}
                                                {!isProcessing && result && (
                                                    <>
                                                        <span className="text-violet-400 font-medium">→ {formatBytes(result.size)}</span>
                                                        {saved !== null && saved > 0 && <span className="font-bold text-emerald-500">−{saved}%</span>}
                                        ...
                                        <button
                                            ...
                                            className="inline-flex items-center gap-1.5 text-xs bg-linear-to-r from-violet-600 to-violet-500 hover:from-violet-700 hover:to-violet-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold px-4 py-1.5 rounded-lg transition-all shrink-0"
                                        >
                                            <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                                <path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                            </svg>
                                    ...
                                    {result && !isProcessing && <NextActions blob={result.blob} filename={result.name} currentTool="strip" />}
                                </div>
                            );
                        })}
                    </div>
