...
function Cross() {
  return (
    <svg className="w-4 h-4 text-fg-3 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
...
function SectionHeader({ children }: { children: React.ReactNode }) {
  return (
    <h2 className="serif italic text-fg-1 m-0 mb-3 pb-1 bd-b-1" style={{ fontSize: 'clamp(21px, 3vw, 32px)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
      {children}
    </h2>
  );
}
    ...
    <main className="bg-page text-fg-1" style={{ padding: 'clamp(48px, 6vw, 80px) 0' }}>
      <div className="max-w-[780px] mx-auto px-4 sm:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 data-animate="hero" className="serif italic text-fg-1 m-0 mb-2" style={{ fontSize: 'clamp(28px, 4vw, 42px)', lineHeight: 1.05, letterSpacing: '-0.025em' }}>Privacy Policy</h1>
          <p data-animate="hero" className="text-[13px] text-fg-3 m-0">Last updated: April 2026</p>
        </div>

        {/* In short box */}
        <div data-animate="scroll" className="bd-2 rounded-[10px] p-5 mb-10 bg-surface">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-[13.5px] font-semibold text-fg-1">In short</span>
          </div>
          <ul className="space-y-2.5">
            {[
              ...
              <li key={item} className="flex items-start gap-2.5 text-[13.5px] text-fg-2">
                <ShieldCheck />
                {item}
              </li>
            ))}
        ...
        <div data-animate-stagger className="space-y-10 text-[13.5px] text-fg-2 leading-[1.75]">
              ...
              <table className="w-full text-[13px]">
                <thead>
                  <tr className="bg-surface">
                    ...
                    <th className="text-left py-2.5 px-4 font-medium text-fg-2">How it handles your images</th>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="py-3 px-4 text-fg-2 bd-t-1">Other apps</td>
                    <td className="py-3 px-4 text-fg-2 bd-t-1">Upload to server, process remotely, store temporarily</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-4 text-fg-2 bd-t-1">ImagePDF.Tools</td>
                    <td className="py-3 px-4 text-accent font-medium bd-t-1">
          ...
          <section>
            ...
            <ul className="mt-3 space-y-3">
              <li className="flex items-start gap-2.5">
                <Check />
                <span><strong className="text-fg-1">Account info</strong> — your name and email address (from Clerk sign-in). Used solely for authentication and account identification.</span>
              </li>
              <li className="flex items-start gap-2.5">
                <Check />
                <span><strong className="text-fg-1">Subscription status</strong> — whether you are on the Free or Pro plan. Stored in Clerk user metadata. No separate database.</span>
              </li>
              <li className="flex items-start gap-2.5">
                <Check />
                <span><strong className="text-fg-1">Anonymous analytics</strong> — page views to understand which features are used, via Plausible Analytics. No personal identifiers, no cross-site tracking.</span>
              </li>
            </ul>
          </section>
          ...
          <section>
            ...
            <ul className="space-y-2.5">
              <li className="flex items-start gap-2.5">
                <Check />
                <span><strong className="text-fg-1">Account info</strong> — to authenticate you, manage your session, and send essential account-related emails (payment receipts, plan changes).</span>
              </li>
              <li className="flex items-start gap-2.5">
                <Check />
                <span><strong className="text-fg-1">Subscription status</strong> — to gate Pro features (unlimited files, ZIP export, ad-free). This data is shown back to you in your account page.</span>
              </li>
              <li className="flex items-start gap-2.5">
                <Check />
                <span><strong className="text-fg-1">Analytics</strong> — to understand product usage patterns and improve the app. No personal data is used for this.</span>
              </li>
            </ul>
          </section>
            ...
            <ul className="mt-3 space-y-2.5">
              {[
                ...
                <li key={bold} className="flex items-start gap-2.5">
                  <Cross />
                  <span><strong className="text-fg-1">{bold}</strong> {rest}</span>
                </li>
              ))}
            </ul>
          </section>
          ...
          <section>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={title} className="flex items-start gap-3 bd-2 rounded-[10px] p-4 bg-surface">
                  <div className="w-9 h-9 rounded-lg bg-accent-dim flex items-center justify-center shrink-0">
                    {icon}
                  </div>
                  <div>
                    <p className="font-medium text-fg-1 text-[13.5px]">{title}</p>
                    <p className="text-fg-2 text-[12.5px] mt-0.5 leading-[1.65]">{desc}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={name} className="flex items-start gap-2.5">
                  <Check />
                  <span><strong className="text-fg-1">{name}</strong> — {detail}</span>
                </li>
              ))}
            </ul>
          </section>
            ...
            <ul className="space-y-2.5">
              {[
                ...
                <li key={right} className="flex items-start gap-2.5">
                  <Check />
                  <span><strong className="text-fg-1">{right}</strong> — {detail}</span>
                </li>
              ))}
            </ul>
          </section>
          ...
          <section>
            ...
            <ul className="mt-3 space-y-2">
              <li className="flex items-start gap-2.5">
                <Check />
                <span><code className="font-mono text-[12px] bg-surface bd-2 px-1.5 py-0.5 rounded">__clerk_*</code> — session cookies required for authentication. Essential; cannot be disabled if you are signed in.</span>
              </li>
              <li className="flex items-start gap-2.5">
                <Check />
                <span><code className="font-mono text-[12px] bg-surface bd-2 px-1.5 py-0.5 rounded">googtrans</code> — stores your selected display language for Google Translate. No tracking data.</span>
              </li>
              <li className="flex items-start gap-2.5">
                <Cross />
          ...
          <section>
            ...
            <p className="mt-3">
              ...
              <a href="mailto:support@imagepdf.tools" className="text-accent hover:underline font-medium">
