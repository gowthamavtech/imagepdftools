...
function Section({ num, title, children }: { num: number; title: string; children: React.ReactNode }) {
  return (
    <section data-animate="scroll" className="mb-8">
      <h2 className="serif italic text-fg-1 m-0 mb-2" style={{ fontSize: 'clamp(21px, 3vw, 32px)', lineHeight: 1.1, letterSpacing: '-0.02em' }}>
        {num}. {title}
      </h2>
...
function SubHeading({ children }: { children: React.ReactNode }) {
  return <p className="font-medium text-fg-1">{children}</p>;
}
    ...
    <main className="bg-page text-fg-1" style={{ padding: 'clamp(48px, 6vw, 80px) 0' }}>
      <div className="max-w-[780px] mx-auto px-8">
        ...
        <div className="mb-10">
          ...
          <h1 data-animate="hero" className="serif italic text-fg-1 m-0 mb-2" style={{ fontSize: 'clamp(28px, 4vw, 42px)', lineHeight: 1.05, letterSpacing: '-0.025em' }}>Terms of Service</h1>
        ...
        <div className="bd-t-1 pt-8">
          ...
          <Section num={8} title="Plans and Pricing">
            ...
            <Ul items={[
              <><strong className="text-fg-1">Free:</strong> Up to 5 files per session, up to 25 MB each. No payment required.</>,
              <><strong className="text-fg-1">Pro Monthly:</strong> $4.99 / month. Unlimited files, up to 100 MB each, ZIP export, format conversion, ad-free. Billed monthly via Stripe.</>,
              <><strong className="text-fg-1">Pro Annual:</strong> $39 / year (~35% savings). Same Pro features, billed annually.</>,
            ]} />
            <p>
